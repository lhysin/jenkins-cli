...
get_latest_version() {
    # Use GitHub releases redirect instead of API to avoid rate limiting
    VERSION=$(curl -fsSLI "https://github.com/${REPO}/releases/latest" | grep -i "location:" | sed -E 's/.*tag\/([^[:space:]]+).*/\1/')
    if [ -z "$VERSION" ]; then
        echo "Error: Failed to get latest version from GitHub" >&2
        echo "Please try again later or download manually from: https://github.com/${REPO}/releases" >&2
        exit 1
    fi
    echo "$VERSION"
}
